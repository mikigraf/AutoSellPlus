...
-- ============================================================
-- Styled UI Helpers (delegates to shared Widgets.lua)
-- ============================================================

local function CreateStyledCheck(parent, size)
    return ns.CreateStyledCheck(parent, size)
end

local function CreateStyledSlider(parent, minVal, maxVal, step)
    return ns.CreateStyledSlider(parent, minVal, maxVal, step)
end

local function CreateStyledEditBox(parent, width)
    return ns.CreateStyledEditBox(parent, width)
end

local function CreateFlatButton(parent, text, width, height)
    return ns.CreateFlatButton(parent, text, width, height)
end
...
local function CreateSettingsOverlay(f)
    ...
    local sections = {
        ...
        },
        {
            title = "Destruction",
            controls = {
                { type = "check", key = "destroyEnabled", label = "Enable Destruction" },
                { type = "cycle", key = "destroyMaxQuality", label = "Max Destroy Quality",
                  options = { {0, "Poor"}, {1, "Common"}, {2, "Uncommon"} } },
                { type = "slider", key = "destroyMaxIlvl", label = "Max Destroy ilvl",
                  min = 0, max = 700, step = 5 },
                { type = "slider", key = "destroyMaxVendorValue", label = "Max Destroy Value",
                  min = 0, max = 100, step = 1, gold = true },
                { type = "slider", key = "destroyConfirmCountdown", label = "Confirm Countdown",
                  min = 1, max = 10, step = 1 },
                { type = "slider", key = "destroyFreeSlotTrigger", label = "Bag Pressure Trigger",
                  min = 0, max = 20, step = 1 },
                { type = "check", key = "destroyProtectTransmog", label = "Protect Transmog" },
                { type = "check", key = "destroyProtectBoE", label = "Protect BoE" },
                { type = "check", key = "destroyProtectEquipmentSets", label = "Protect Equipment Sets" },
            },
        },
    }
